...
*/
	...
	public function result()
	{
		if (permissionChecker('onlineexamreport')) {
			$onlineExamUserStatusID = htmlentities((string) escapeString($this->uri->segment(3)));
			if ((int) $onlineExamUserStatusID !== 0) {
	...
	public function ranking($onlineexamID, $userID, $examtimeID)
	{
		...
		$position = '';
		if (inicompute($returnArray) && isset($returnArray[$onlineexamID][$userID][$examtimeID]['rank'])) {
